...
public class Datasource
{
    ...
    public static final String CONNECTION_STRING = "jdbc:sqlite:D:\\Java Masterclass\\MusicDatabase\\" + DB_NAME;
    ...
    public static final String CREATE_ARTIST_FOR_SONG_VIEW =
            "CREATE VIEW IF NOT EXISTS " + TABLE_ARTIST_SONG_VIEW + " AS " +
                    "SELECT " + TABLE_ARTISTS + '.' + COLUMN_ARTIST_NAME + " AS " + COLUMN_ALBUM_ARTIST + ',' + TABLE_ALBUMS + '.' + COLUMN_ALBUM_NAME + " AS " + COLUMN_SONG_ALBUM + ',' + TABLE_SONGS + '.' + COLUMN_SONG_TRACK + ',' + TABLE_SONGS + '.' + COLUMN_SONG_TITLE + " FROM " + TABLE_SONGS +
                    " INNER JOIN " + TABLE_ALBUMS + " ON " + TABLE_SONGS + '.' + COLUMN_SONG_ALBUM + " = " + TABLE_ALBUMS + '.' + COLUMN_ALBUM_ID +
                    " INNER JOIN " + TABLE_ARTISTS + " ON " + TABLE_ALBUMS + '.' + COLUMN_ALBUM_ARTIST + " = " + TABLE_ARTISTS + '.' + COLUMN_ARTIST_ID +
    ...
    public static final String QUERY_VIEW_SONG_INFO =
            "SELECT " + COLUMN_ALBUM_ARTIST + ',' + COLUMN_SONG_ALBUM + ',' + COLUMN_SONG_TRACK + " FROM " + TABLE_ARTIST_SONG_VIEW +
                    " WHERE " + COLUMN_SONG_TITLE + " = \"";

    public static final String QUERY_VIEW_SONG_INFO_PREP = "SELECT " + COLUMN_ALBUM_ARTIST + ',' + COLUMN_SONG_ALBUM + ',' + COLUMN_SONG_TRACK + " FROM " + TABLE_ARTIST_SONG_VIEW +
            " WHERE " + COLUMN_SONG_TITLE + " = ?";
    ...
    public List<SongArtist> queryArtistForSong(String songName, int sortOrder)
    {
        ...
        {
            System.out.println("Query artists for song failed: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }

    //Meta Data (.schema like) tutorial
    public void querySongsMetaData()
    {
             ...
             ResultSet results = statement.executeQuery(sql))
        {
            ...
            int numClomunss = metaData.getColumnCount();
            for (int i = 1; i <= numClomunss; i++)
            {
                System.out.format("Column %d in the Songs Table is named %s \n", i, metaData.getColumnName(i));
            }
        } catch (SQLException e)
        {
            System.out.println("Failed querying for meta data: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void getCount(String table)
    {
        ...
        {
            System.out.println("Couldn't execute function counting: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public List<SongArtist> querySongInfoView(String title)
    {
        ...
        {
            System.out.println("Query of View failed: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
    ...
    private int insertArtist(String name) throws SQLException
    {
        ...
        } else
        {
            ...
            {
                throw new SQLException("Couldn't get ID for new artist!");
            }
        }
    }
    ...
    private int insertAlbum(String name, int artistId) throws SQLException
    {
        ...
        } else
        {
            ...
            {
                throw new SQLException("Couldn't get ID for new album!");
            }
        }
    }
    ...
    public void insertSong(String title, String artist, String album, int track)
    {
        ...
        } catch (Exception e)
        {
            ...
            try
            {
                //performing ROLLBACK in case of an error
                conn.rollback();
            } catch (SQLException e2)
            {
                //Oh boy... things are really bad
                System.out.println("Error with rollback: " + e.getMessage());
            }
        } finally
        {
            try
            {
                System.out.println("Resetting commit default behaviour.");
                conn.setAutoCommit(true);
            } catch (SQLException e)
            {
                System.out.println("Couldn't reset commit default behaviour: " + e.getMessage());
            }
        }
    }
}
